    ...
    (() => {
        ...
        function menuInit() {
            if (document.querySelector(".icon-menu")) document.addEventListener("click", (function(e) {
                if (bodyLockStatus && e.target.closest(".icon-menu")) {
                    bodyLockToggle();
                    document.documentElement.classList.toggle("backdrop-show");
                    document.documentElement.classList.toggle("menu-open");
                }
                document.querySelector(".menu__content");
                if (document.documentElement.classList.contains("menu-open") && !e.target.closest(".menu__content") && !e.target.closest(".icon-menu")) {
                    functions_menuClose();
                    document.documentElement.classList.remove("backdrop-show");
                }
        ...
        class Popup {
            ...
            close(selectorValue) {
                ...
                const iframe = this.previousOpen.element.querySelector("iframe");
                ...
                on: {
                    slideChange: function() {
                        ...
                        indicators.forEach((indicator => {
                            ...
                            if (span) span.style.left = "";
                        }));
                        indicators.forEach(((indicator, index) => {
                            const leftPercentage = (index + 1) * percentagePerSlide;
                            const span = indicator.querySelector("span");
                            if (span) span.dataset.leftValue = `${leftPercentage}%`;
                            if (index <= realIndex) {
                                indicator.classList.add("active");
                                if (span) span.style.left = span.dataset.leftValue;
                            }
                        }));
                    }
                }
            });
            if (document.querySelector(".product-thumbs__slider")) var swiperThumbs = new swiper_core_Swiper(".product-thumbs__slider", {
                observer: true,
                observeParents: true,
                slidesPerView: "auto",
                spaceBetween: 12,
...
*/
        ...
        const karaokeSpan = document.querySelector(".karaoke span");
        gsapWithCSS.to(karaokeSpan, {
        ...
        mm.add("(min-width: 768px)", (() => {
            document.querySelectorAll(".support-block").forEach(((block, index, blocks) => {
                const imageBody = block.querySelector(".support-block__image");
                const img = block.querySelector("img");
                const imageBodyHeight = imageBody.offsetHeight;
                const imageHeight = img.offsetHeight;
                const differenceHeight = imageBodyHeight - imageHeight - 24;
                block.offsetHeight;
                let height = imageBodyHeight * index;
                gsapWithCSS.set(imageBody, {
                    y: `-${height}%`,
                    z: 0
                });
                ...
                gsapWithCSS.to(img, {
                    scrollTrigger: {
                        ...
                        pin: true,
                        start: "top 20px",
                        end: isLastBlock ? `+${height + differenceHeight / imageBodyHeight * 100}% top` : `+${height + 150}% top`,
                        pinType: "transform",
                        pinSpacing: false,
                        scrub: 1
                    }
        ...
        const buttons = document.querySelectorAll(".button-contacts-main");
        const contacts = document.querySelector(".contacts");
        const closeButton = document.querySelector(".contacts-close");
        buttons.forEach((button => {
            button.addEventListener("click", (event => {
                ...
                if (contacts.classList.contains("active")) {
                    bodyLock();
                    document.documentElement.classList.add("backdrop-show");
                    functions_menuClose();
                } else {
                    bodyUnlock();
                    document.documentElement.classList.remove("backdrop-show");
        ...
        document.addEventListener("click", (event => {
            if (contacts.classList.contains("active") && !contacts.contains(event.target) && !Array.from(buttons).some((button => button.contains(event.target)))) {
                contacts.classList.remove("active");
                bodyUnlock();
                document.documentElement.classList.remove("backdrop-show");
            }
        }));
        const categories = document.querySelectorAll(".item-blog-categories");
        categories.forEach((category => {
            category.addEventListener("click", (() => {
                document.querySelector(".item-blog-categories.active")?.classList.remove("active");
